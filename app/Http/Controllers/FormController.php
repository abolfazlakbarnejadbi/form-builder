...
class FormController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('form_construction.index', compact('forms'));
    }
    ...
    {
        return view('form_construction.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        //Empty check title not sent
        if ($request->title == null) {
            return response()->json(['status' => false, 'message' => 'عنوان باید مقدار داشته باشد']);
        }
        ...
        if ($forms->length > 0) {
            ...
            try {
                ...
                return response()->json(['status' => true, 'message' => 'فرم جدید با موفقیت ساخته شد', 'redirect_url' => route('form.index')]);
            } catch (\Exception $e) {
                Log::warning('خطا ساخت فرم: ' . $e->getMessage());
                // return redirect()->route('form.index')->with('error', 'ساخت فرم با خطا مواجه شد ');
                return response()->json(['status' => false, 'message' => 'ساخت فرم با خطا مواجه شد']);
            }
        }
    }
    ...
    public function show(Form $form)
    {
        ...
        $single_responses = FormResponse::where('form_id', $form->id)->OrderBy('created_at', 'desc')->get();
        $row_responses = $single_responses->unique('response_group');

        $count_today_responses = FormResponse::where('form_id', $form->id)->whereDate('created_at', Carbon::today())->get()->unique('response_group')->count();

        $chenge_seen_status = FormResponse::where('form_id', $form->id)
            ->update(['seen' => 1]);
        return view('form_construction.form_response', compact('row_responses', 'form', 'count_today_responses', 'single_responses'));
    }
}
